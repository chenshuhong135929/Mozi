...
var Class = (function() {
	...
	function c(k) {
		...
		for (var d = 0, g = e.length; d < g; d++) {
			var j = e[d], h = k[j];
			if (f && Object.isFunction(h)
					&& h.argumentNames().first() == "$super") {
				var l = h;
				h = (function(i) {
					return function() {
...
Object.extend(Function.prototype, (function() {
	...
	function g() {
		var l = this.toString().match(/^[\s\(]*function[^(]*\(([^)]*)\)/)[1]
				.replace(/\/\/.*?[\r\n]|\/\*(?:.|[\r\n])*?\*\//g, "").replace(
						/\s+/g, "").split(",");
...
Object.extend(String.prototype, (function() {
	...
	function underscore() {
		return this.replace(/::/g, "/").replace(/([A-Z]+)([A-Z][a-z])/g,
				"$1_$2").replace(/([a-z\d])([A-Z])/g, "$1_$2").replace(/-/g,
				"_").toLowerCase()
	}
	function dasherize() {
...
var Template = Class.create({
	...
	evaluate : function(a) {
		...
		return this.template.gsub(this.pattern, function(d) {
			...
			while (d != null) {
				var c = d[1].startsWith("[") ? d[2].replace(/\\\\]/g, "]")
						: d[1];
				b = b[c];
				if (null == b || "" == d[3]) {
					break
...
Element.Methods = {
	...
	update : (function() {
		...
		var d = (function() {
			var f = document.createElement("script"), h = false;
			try {
				f.appendChild(document.createTextNode(""));
				h = !f.firstChild || f.firstChild
						&& f.firstChild.nodeType !== 3
			} catch (g) {
				h = true
			}
			f = null;
		...
		function c(f, g) {
			...
			if (b || a) {
				if (e in Element._insertionTranslations.tags) {
					...
					Element
							._getContentFromAnonymousElement(e,
									g.stripScripts()).each(function(h) {
								f.appendChild(h)
							})
				} else {
	...
	siblings : function(a) {
		a = $(a);
		return Element.previousSiblings(a).reverse().concat(
				Element.nextSiblings(a))
	},
	match : function(b, a) {
		if (Object.isString(a)) {
			a = new Selector(a)
	...
	down : function(b, c, a) {
		...
		return Object.isNumber(c) ? Element.descendants(b)[c] : Element.select(
				b, c)[a || 0]
	},
...
var Selector = Class
		.create({
			...
			shouldUseXPath : (function() {
				var a = (function() {
					...
					if (document.evaluate && window.XPathResult) {
						...
						var b = document.evaluate(c, d, null,
								XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null);
				...
				return function() {
					...
					if (Prototype.Browser.WebKit
							&& (b.include("-of-type") || b.include(":empty"))) {
						return false
					}
					if ((/(\[[\w-]*?:|:checked)/).test(b)) {
						return false
			...
			compileMatcher : function() {
				...
				while (e && le != e && (/\S/).test(e)) {
					...
					for (var i = 0; i < len; i++) {
						...
						if (m = e.match(p)) {
							this.matcher
									.push(Object.isFunction(c[name]) ? c[name]
											(m) : new Template(c[name])
											.evaluate(m));
							e = e.replace(m[0], "");
							break
						}
			...
			compileXPathMatcher : function() {
				var h = this.expression, j = Selector.patterns, c = Selector.xpath, g, b, a = j.length, d;
				...
				while (h && g != h && (/\S/).test(h)) {
					...
					for (var f = 0; f < a; f++) {
						d = j[f].name;
						if (b = h.match(j[f].re)) {
							this.matcher.push(Object.isFunction(c[d]) ? c[d](b)
									: new Template(c[d]).evaluate(b));
							h = h.replace(b[0], "");
							break
						}
					}
...
if (Prototype.BrowserFeatures.SelectorsAPI
	...
	Selector.CASE_INSENSITIVE_CLASS_NAMES = (function() {
		var c = document.createElement("div"), a = document
				.createElement("span");
				...
				Selector,
				{
					...
					xpath : {
						...
						pseudos : {
							...
							not : function(f) {
								var j = f[6], c = Selector.patterns, k = Selector.xpath, a, l, h = c.length, b;
								var d = [];
...
Form.Element.Methods = {
	...
	activate : function(a) {
		...
		try {
			a.focus();
			if (a.select
					&& (a.tagName.toLowerCase() != "input" || !(/^(?:button|reset|submit)$/i
							.test(a.type)))) {
				a.select()
			}
		} catch (b) {
...
(function() {
	...
	function c(z) {
		...
		if (A && A.tagName) {
			if (x === "load"
					|| x === "error"
					|| (x === "click" && A.tagName.toLowerCase() === "input" && A.type === "radio")) {
				y = A
			}
		}
	...
	function b(z) {
		var y = document.documentElement, x = document.body || {
			scrollLeft : 0
		};
		return z.pageX
	...
	function a(z) {
		...
		return z.pageY
				|| (z.clientY + (y.scrollTop || x.scrollTop) - (y.clientTop || 0))
	}
	function n(x) {
		v.extend(x);
		x.preventDefault();
	...
	document.getElementsByClassName = function(b) {
		...
		b.getElementsByClassName = Prototype.BrowserFeatures.XPath ? function(
			...
			for (var d = 0, k, j; k = c[d]; d++) {
				if (k.className
						&& (j = " " + k.className + " ")
						&& (j.include(f) || (h && h.all(function(i) {
							return !i.toString().blank()
									&& j.include(" " + i + " ")
						})))) {
